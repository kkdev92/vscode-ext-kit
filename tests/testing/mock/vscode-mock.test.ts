 ...
 *
 * Add focused cases when the public mock gains a value class, namespace state
 * or builder behavior not already exercised by framework adapter tests. Keep
 * the assertions within the mock's documented partial scope; platform behavior
 * outside it belongs in the Extension Host lane.
 */
...
// Observable value-class contracts that production adapters depend on. Each
// assertion states the behavior being mirrored; implementation details outside
// these contracts are deliberately not part of the mock.
...
/**
 * The details a hand-rolled `vscode` stub gets wrong.
 *
 * Each of these is a place where the obvious mock is subtly kinder than the
...
describe('faithfulness to the real API', () => {
  ...
  it("includes workspace.applyEdit — the kit's own editor helpers call it", async () => {
    ...
    await expect(mock.workspace.applyEdit(new mock.WorkspaceEdit())).resolves.toBe(true);
  });

  it('parses the scheme out of Uri.parse', () => {
    const mock = createVSCodeMock(vi);
    const uri = mock.Uri.parse('untitled:Untitled-1');
    expect(uri.scheme).toBe('untitled');
    expect(mock.Uri.parse('https://example.com/x').scheme).toBe('https');
  });

  it('detaches an event listener when its subscription is disposed', () => {
    const mock = createVSCodeMock(vi);
    const emitter = new mock.EventEmitter<number>();
    const seen: number[] = [];
    const subscription = emitter.event((value: number) => seen.push(value));
  ...
  it('fires onDidHide from hide() and from disposing a visible quick input', () => {
    const mock = createVSCodeMock(vi);

    const quickPick = mock.window.createQuickPick();
    const pickHides = vi.fn();
    quickPick.onDidHide(pickHides);
    quickPick.show();
    quickPick.hide();
    expect(pickHides).toHaveBeenCalledTimes(1);
    quickPick.show();
    quickPick.dispose();
    expect(pickHides).toHaveBeenCalledTimes(2);

    const inputBox = mock.window.createInputBox();
    const inputHides = vi.fn();
    inputBox.onDidHide(inputHides);
    inputBox.show();
    inputBox.dispose();
    expect(inputHides).toHaveBeenCalledTimes(1);
    // Disposing an already-hidden input must not fire again.
    inputBox.dispose();
    expect(inputHides).toHaveBeenCalledTimes(1);
  });
});
